...
pipeline {
    ...
    stages {
        ...
        stage("Exporting proxies from Apigee") {
            steps {
            script {
            def currentDate = new Date().format("dd-MM-yyyy")
                    def exportDir = "<APIGEE_ORG_NAME>/${currentDate}/proxies"
                    
                    sh """
                    mkdir -p ${exportDir}
                    ${APIGEE_CLI_DIR}/apigeecli proxies export -o <APIGEE_ORG_NAME> -t ${TOKEN} -f ${exportDir}
                    """
            ...
            if (changes) {
                sh "git commit -m 'Added proxies from Jenkins Pipeline'"
                withCredentials([gitUsernamePassword(credentialsId: "${GITHUB_CREDS}", gitToolName: 'Default')]) {
                    sh "git push -u origin main"
                }
            } else {
                echo "No changes to commit in proxies. Skipping git push."
                    }
                }
            }
        }
